...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
  ...
  if (collThrustCmd > 0.0) {
    ...
    float b_x_target = -CONSTRAIN(accelCmd[0] / c_d, -maxTiltAngle, maxTiltAngle);
    float b_y_target = -CONSTRAIN(accelCmd[1] / c_d, -maxTiltAngle, maxTiltAngle);
    ...
    float b_x_err = b_x_target - b_x;
    float b_y_err = b_y_target - b_y;

    float b_x_p_t = kpBank * b_x_err;
    float b_y_p_t = kpBank * b_y_err;

    pqrCmd[0] = R21 * b_x_p_t - R11 * b_y_p_t;
    pqrCmd[1] = R22 * b_x_p_t - R12 * b_y_p_t;
